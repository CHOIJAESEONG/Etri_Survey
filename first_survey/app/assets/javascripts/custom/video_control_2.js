...
function Class_Modal(vid){
	...
	this.set_persent=function(current){
		persent.text((current_progress/totalShot*100).toFixed(2)+'%');
...
function Class_SurveyForm(){
	...
	var ajax_get_data=function(data){
		...
		if(video_link=='/assets/'+data[0].media_url) {
			...
			if(data[0].is_last==true){
				//last shot
				location.replace("http://211.253.24.242/")
				return;
			}

			//처음이거나 해당 영상의 샷을 다 진행해서 다음 링크를 받은경우
			video_link='/assets/'+data[0].media_url;
			start_list=data[0].start_time_list;
			end_list=data[0].end_time_list;
			shot_id_list=data[0].shot_id_List;
			cid=data[0].media_id;
			filename=data[0].program_name;
			totalShot=data[0].total_shot;
			current_progress=data[0].current_progress;
	...
	next_btn.click(function(){
		//alert($("#textarea_reason").val());
			if($("#textarea_reason").val().length==0){
				alert('사유를 입력해주세요');
				return false;
			}
